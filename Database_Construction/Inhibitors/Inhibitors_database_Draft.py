...
con = sqlite3.connect('INHIBITOR_KINASE_DATABASE_DRAFT_99.db')
...
cur.execute("CREATE TABLE inhibitor_references_1(Key PRIMARY KEY, ChEMBL_ID_, TARGETS, REFERENCE VARCHAR(200), PUBMED_ID VARCHAR(30))") # use your column names here
...
cur.executemany("INSERT INTO inhibitor_references_1(Key,ChEMBL_ID_,TARGETS,REFERENCE, PUBMED_ID) VALUES (?,?,?,?,?);", to_in_2)
con.commit()  #Commit the changes 
...
cur.execute("SELECT ChEMBL_ID FROM inhibitor_information INNER JOIN inhibitor_references_1 ON inhibitor_references_1.ChEMBL_ID_ = inhibitor_information.ChEMBL_ID ;") 
