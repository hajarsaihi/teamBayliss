...
con = sqlite3.connect('PROTEIN_KINASE_DATABASE_DRAFT_2.db')
...
cur.execute("CREATE TABLE Kinase_Information_1(key_numbers, Name  PRIMARY KEY, Groups TEXT, Family VARCHAR(30), Subfamily VARCHAR(30), Entrez_GeneID VARCHAR(30), Entrez_description REAL, location, Alias);") # use your column names here
...
cur.executemany("INSERT INTO Kinase_Information_1(key_numbers,Name, Groups, Family, Subfamily, Entrez_GeneID, Entrez_description,location, Alias) VALUES (?,?,?,?,?,?,?,?,?);", to_db_1)
...
cur.execute("SELECT Name FROM Kinase_Information_1 INNER JOIN Kinase_Phosphosite_1 ON Kinase_Phosphosite_1.GENE = Kinase_Information_1.Name ;") 

cur.execute("SELECT Name FROM Kinase_Information_1 INNER JOIN InteractingKinase_1 ON InteractingKinase_1.Target_Kinase = Kinase_Information_1.Name ;") 
