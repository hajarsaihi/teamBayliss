...
#for file_name in os.listdir("."):
  # check if it ends with .tsv
    ...
    # open the file, rename columns
    d_cols = ["Substrate", "Control_mean", "Inhibitor_mean", "Fold_change", "p_value", "ctrlCV", "treatCV"]
    d = pd.read_csv(filename, names=d_cols, na_values='inf', sep='\t', skiprows=[0]) #C - drops row 0 which has old headers
    d["ctrlCV"].fillna(9.9, inplace=True) #C - fill NaN w 9.9, if User don't provide CV,
    d["treatCV"].fillna(9.9, inplace=True) #C - default CV_P 10.0 will display all results
    return d
